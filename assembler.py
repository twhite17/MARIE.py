...
import convert # a small library for converting data types
...
class Assembler:
	...
	def load(self, filePath=None):
		...
		# Removes Comments from the marie code.
		ntext = []
		for line in text:
			nline = ""
			for ch in line:
		...
		# Removes empty lines so program takes up less space in memory
        # otherwise the compiled result would have a bunch of memory locations with nothing in them
		self.code = []
		for line in text:
			if line != "":
        ...
        # there isn't any code yet, this is just data structure setup
		self.build = ["" for i in range(len(self.code))]


	def getDirectives(self):

		ADRLookup = {}    # address that need to be know for variable lookup
		...
		for lineNum in range(len(self.code)):
			...
			try:
				...
				if "," in line[0]:
                    ...
                    # Converts all constants to integer representations
